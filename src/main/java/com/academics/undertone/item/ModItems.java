...
// ** After doing so here, you must make JSON files using that unique id under resources/assets/undertone in the models and textures folders to define the appearance of the item in game.
// ** You can copy the JSON files for an existing item and change the name and texture reference to match your new item, which is usually easier than writing them from scratch.
// ** For a simple item like an ingot, you can use the existing iron_ingot JSON files as a template, or one of the ones already made.
...
// ** Each unique "thing" should go in its own folder / class, so items go in the item folder and blocks go in the block folder, and so on.
// ** This is not strictly necessary but it helps keep things organized and easier to find.
// ** READ ME ** \\
// ** ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ ** \\
